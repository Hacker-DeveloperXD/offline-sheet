(globalThis.TURBOPACK = globalThis.TURBOPACK || []).push(["static/chunks/src_b30a5fae._.js", {
...
function SpreadsheetProvider({ children }) {
        ...
        "SpreadsheetProvider.useCallback[setSpreadsheetWithHistory]": (updater, actionSource)=>{
            setInternalSpreadsheet({
                "SpreadsheetProvider.useCallback[setSpreadsheetWithHistory]": (current)=>{
                    ...
                    if (actionSource !== 'undo' && actionSource !== 'redo' && current && newSpreadsheet && JSON.stringify(current) !== JSON.stringify(newSpreadsheet)) {
    ...
    // This is the `setSpreadsheet` exposed to consumers if they need direct state setting without history (e.g., initial load)
    // For most operations, internal functions should call setSpreadsheetWithHistory.
    const exposedSetSpreadsheet = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "SpreadsheetProvider.useCallback[exposedSetSpreadsheet]": (value)=>{
            setInternalSpreadsheet(value); // Directly set, e.g. after loading from DB
        }
    ...
    const loadSpreadsheet = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "SpreadsheetProvider.useCallback[loadSpreadsheet]": async (id)=>{
            ...
            if (result.success && result.data) {
                setInternalSpreadsheet(result.data);
            } else {
                setInternalSpreadsheet(null);
                toast({
                    title: "Error",
    ...
    const saveSpreadsheet = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "SpreadsheetProvider.useCallback[saveSpreadsheet]": async ()=>{
            ...
            if (result.success) {
                setInternalSpreadsheet(updatedSpreadsheet); // Ensure local state has updatedAt
                toast({
                    title: "Success",
                                        ...
                                        if (rIdx === rowIndex) {
                                            return row.map({
                                                "SpreadsheetProvider.useCallback[updateCell].newSheets.newCells": (cell, cIdx)=>{
                                                    if (cIdx === colIndex) {
                                                        let value = newCellData.rawValue;
                                                        if (typeof newCellData.rawValue === 'string' && newCellData.rawValue.startsWith('=')) {
                                                            value = `Formula: ${newCellData.rawValue.substring(1)}`; // Placeholder
                                                        }
                                                        return {
                                                            ...cell,
                                                            ...newCellData,
                                            ...
                                            return row.map({
                                                "SpreadsheetProvider.useCallback[updateSelectedCellStyle].newSheets.newCells": (cell, cIdx)=>{
                                                    if (cIdx === colIndex) {
                                                        ...
                                                        // Remove style property if it's set to false/undefined (e.g. unbold)
                                                        // and was the default (e.g. bold: false is default)
                                                        // For simplicity, we'll just merge. Refinement could be done later.
                                                        return {
                                                            ...cell,
                                                            style: newStyle
                                                        };
    ...
    const undo = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "SpreadsheetProvider.useCallback[undo]": ()=>{
            if (undoStack.length > 0) {
                ...
                if (internalSpreadsheet) {
                    // Create a deep copy for the redo stack
                ...
                setInternalSpreadsheet(previousState);
                setSelectedCell(null); // Reset selection as context might have changed
            }
        }
    }["SpreadsheetProvider.useCallback[undo]"], [
        undoStack,
    ...
    const redo = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "SpreadsheetProvider.useCallback[redo]": ()=>{
            if (redoStack.length > 0) {
                ...
                setInternalSpreadsheet(nextState);
...
_s(SpreadsheetProvider, "nV3qLw9FsQv9D6jtmG5S34P0Tpo=", false, function() {
    return [
...
function Toolbar({ spreadsheetName }) {
    _s();
    const { saveSpreadsheet, spreadsheet, setSpreadsheet, selectedCell, updateSelectedCellStyle, undo, redo, canUndo, canRedo } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$hooks$2f$useSpreadsheet$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useSpreadsheet"])();
    ...
    const insertRow = ()=>console.log("Insert Row (Not implemented)");
    const insertColumn = ()=>console.log("Insert Column (Not implemented)");
    const mergeCells = ()=>console.log("Merge Cells (Not implemented)");
    // const setFontColor = () => console.log("Set Font Color (Not implemented)"); // Removed
    // const setBackgroundColor = () => console.log("Set Background Color (Not implemented)"); // Removed
...
_s(Toolbar, "7xWozohBBu09FBpNGVVotG36Jc4=", false, function() {
    return [
...
const DEFAULT_ROW_COUNT = 6969; // Initial rows - WARNING: Performance issues expected
const DEFAULT_COLUMN_COUNT = 6969; // Initial columns - WARNING: Performance issues expected
...
//# sourceMappingURL=src_b30a5fae._.js.map